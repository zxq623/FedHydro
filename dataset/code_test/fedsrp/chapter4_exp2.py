...
model_name = '/home/FedHydro/dataset/code_test/fedsrp/basic_local_model.model'
print(model_name)
learning_rate = 0.001
...
date_range1 = {# 03
        ...
        'val_date': {
            'start_date': pd.to_datetime("2004-10-01", format="%Y-%m-%d"),
            'end_date': pd.to_datetime("2006-09-30", format="%Y-%m-%d")  # 测试日期1，差异性大， 数据太少不设置验证期
        },
        'test_date': {
...
def train_global_model_with_sparse(global_epoch, global_batch_size, block_ids, sparse_train_x, sparse_train_y):
    data_path = "/home/FedHydro/dataset/series_data"
    unit_code = "03"
    ...
    # plt.ylabel('Loss')
    # plt.show()
    model.save('./global_model_'+str(node_count)+'basins_'+str(global_epoch)+'epoch_2years.model')
...
def mutil_fune(global_model_name, basin_id, single_basin_data, epoch, batch_size, the_data_range):
    ...
    mutil_fune_model.save('/home/FedHydro/dataset/code_test/fedsrp/models/mutil_fune'+str(basin_id)+'_2years.model')
...
def transfer_a(global_model_name, basin, single_basin_data, epoch, batch_size, test_x, test_y, ds_test):
    ...
    tl_a_model.save('/home/FedHydro/dataset/code_test/fedsrp/models/tl_a_basin'+str(basin)+'_2years.model')
...
def transfer_b(global_model_name, basin, single_basin_data, epoch, batch_size, test_x, test_y, ds_test):
    ...
    tl_b_model.save('/home/FedHydro/dataset/code_test/fedsrp/models/tl_b_basin' + str(basin) + '_2years.model')
...
def train_global_model():
    ...
    sparse_train_x = train_x1
    sparse_train_y = train_y1
    for epoch in epochs:
